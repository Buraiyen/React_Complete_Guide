...
- in the assignment, we created a *controlled component* for ~Expenses~, and it's not the first time we did that
  + whenever we do two-way binding, we are controlling a component
  + this means a value used in a component is passed on to a parent component, AND it is received from the parent component
  + ~Expenses~ controls ~ExpenseFilter~

- stateful vs. stateless components. What do they mean?
  + in all React apps you build, you will have components that manage some state
  + you will have components that don't manage any state. These kind of components are just there to output data
...
- note, this is because calling ~useState~ will create a new state
...
9. What's wrong with this code snippet?
